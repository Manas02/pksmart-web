...
if st.button("Run PKSmart"):
    with st.spinner('Predicting PK parameters...'):
        if smiles_list:                 
            ...
            st.dataframe(animal_predictions[['smiles_r'] + animal_columns], hide_index=True)
            ...
            with open("../features_mfp_mordred_animal_artificial_human_modelcolumns.txt") as f:
                model_features = f.read().splitlines()

            human_predictions['smiles_r'] = data_mordred['smiles_r']

            human_predictions['VDss'] = predict_VDss(data_mordred, model_features)
            Vd_Tc = ts_data["human_VDss_L_kg"]
            with open(f"../folderror_human_VDss_L_kg_generator.sav", 'rb') as f:
            ...
            human_predictions['CL'] = predict_CL(data_mordred, model_features)
            CL_Tc =  ts_data["human_CL_mL_min_kg"]
            with open(f"../folderror_human_CL_mL_min_kg_generator.sav", 'rb') as f:    
                loaded = pickle.load(f)
            ...
            with open(f"../folderror_human_fup_generator.sav", 'rb') as f:
                loaded = pickle.load(f)
            ...
            human_predictions['MRT'] = predict_MRT(data_mordred, model_features)
            MRT_Tc =  ts_data["human_mrt"]
            with open(f"../folderror_human_mrt_generator.sav", 'rb') as f:
                loaded = pickle.load(f)
            ...
            human_predictions['thalf'] = predict_thalf(data_mordred, model_features)
            thalf_Tc =  ts_data["human_thalf"]
            with open(f"../folderror_human_thalf_generator.sav", 'rb') as f:
                loaded = pickle.load(f)
            ...
            st.dataframe(human_predictions, hide_index=True)
